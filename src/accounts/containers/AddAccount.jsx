...
const AddAccount = ({ order, accType }) => {
  ...
  const currencyOptions = order
    ? currencies
        .filter((currency) => currency.id === order.currencyReceivedId)
        .map((currency) => ({ label: `${currency.name} (${currency.ISO === 'PEN' ? 'S/.' : '$'})`, value: currency.id }))
    : currencies.map((currency) => ({ label: `${currency.name} (${currency.ISO === 'PEN' ? 'S/.' : '$'})`, value: currency.id }));
