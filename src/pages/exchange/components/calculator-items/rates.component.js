    ...
    <div className={classes.Rates}>
      ...
      <div className="flex items-center justify-center">
        <div className={`${classes.BuyCard} ${conditionToBuy ? classes.Faint : ""}`}>
          <h4>Compramos: {convertRate(conditionCouponRates ? couponRates.buy : actualRates.buy)}</h4>
        </div>
        <span className={classes.RatesDivider} />
        <div className={`${classes.SellCard} ${conditionToSell ? classes.Faint : ""}`}>
          <h4>Vendemos: {convertRate(conditionCouponRates ? couponRates.sell : actualRates.sell)}</h4>
        </div>
      </div>
    </div>
  );
